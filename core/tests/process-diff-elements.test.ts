...
function makeToken(index: number, flags: number, el: Node): Token {
	return {
		index,
		flags,
		textOffset: index,
...
describe("processDiffElements handleDiff branch coverage", () => {
	...
	it("leftCount=0 (pure ADDED): leftRange in left editor, rightRange in right editor", async () => {
		...
		const leftTokens = [makeToken(0, TOKEN_FLAGS_TYPE_TEXT | TOKEN_FLAGS_LINE_START, leftEl.childNodes[0]!)];
		const left = {
			el: leftEl,
			tokens: leftTokens,
	...
	it("emptyStart=0: falls back to contentElement when marker creation is blocked", async () => {
		...
		const rightTokens = [makeToken(0, TOKEN_FLAGS_TYPE_TEXT | TOKEN_FLAGS_LINE_START, rightEl.childNodes[0]!)];
...
describe("processDiffElements edge cases: mixed structural + content", () => {
	it("left has structural then content, right has content only: only content part diffed", async () => {
		...
		const left = makeEditorWithMixedTokens([
			TOKEN_FLAGS_TYPE_STRUCTURAL,
			TOKEN_FLAGS_TYPE_TEXT,
			TOKEN_FLAGS_TYPE_TEXT,
		]);
...
describe("processDiffElements edge cases: empty documents", () => {
	it("both sides empty: no diffs", async () => {
		...
		const left = {
			el: leftEl,
			tokens: [] as Token[],
			snapshot: makeSnapshot([], leftEl),
			editor: makeMockEditor([], leftEl),
		};
		const right = {
			el: rightEl,
			tokens: [] as Token[],
			snapshot: makeSnapshot([], rightEl),
			editor: makeMockEditor([], rightEl),
		};
	...
	it("left empty, right has many tokens: all ADDED", async () => {
		...
		const ctx = await runProcessDiffElements(
			left,
			right,
			[],
			[
	...
	it("non-adjacent left-empty diffs should NOT be merged (content tokens only)", async () => {
		...
		const leftTokens: Token[] = [
			makeToken(0, TOKEN_FLAGS_LINE_START | TOKEN_FLAGS_TYPE_TEXT, leftEl.childNodes[0]!),
			{
				...
				textOffset: 1,
				textLength: 1,
				startNode: leftSharedTextNode,
				startOffset: 0,
				endNode: leftSharedTextNode,
				endOffset: 1,
				lineNumber: 0,
				containerIndex: 0,
			},
	...
	it("non-adjacent left-empty diffs should NOT be merged (with structural tokens)", async () => {
		...
		const leftTokens: Token[] = [
			makeToken(0, TOKEN_FLAGS_LINE_START | TOKEN_FLAGS_TYPE_STRUCTURAL, leftEl.childNodes[0]!),
			{
				...
				textOffset: 0,
				textLength: 1,
				startNode: sharedTextNode,
				startOffset: 0,
				endNode: sharedTextNode,
				endOffset: 1,
				lineNumber: 0,
				containerIndex: 0,
			},
	...
	it("adjacent left-empty diffs CAN still be merged when emptyEl is null", async () => {
		...
		const rightTokens: Token[] = [
			makeToken(0, TOKEN_FLAGS_LINE_START | TOKEN_FLAGS_TYPE_TEXT, rightEl.childNodes[0]!),
			{
				...
				textOffset: 1,
				textLength: 1,
				startNode: addedElementTextNode,
				startOffset: 0,
				endNode: addedElementTextNode,
				endOffset: 1,
				lineNumber: 0,
				containerIndex: 0,
			},
			{
				...
				textOffset: 2,
				textLength: 1,
				startNode: trailingTextNode,
				startOffset: 0,
				endNode: trailingTextNode,
				endOffset: 1,
				lineNumber: 0,
				containerIndex: 0,
			},
