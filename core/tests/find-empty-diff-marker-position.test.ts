...
function makeLineBoundary(which: Node, where: InsertPosition, containerIndex: number): LineBoundaryInfo {
...
describe("Case A — empty document", () => {
	...
	it("lineBoundaries[1]이 없으면 [0]을 사용", () => {
		...
		const result = call([makeToken({ flags: TOKEN_FLAGS_LINE_START, containerIndex: 1 })], 0, [], [lb0], 0);
		expect(result?.which).toBe(n0);
		expect(result?.where).toBe("afterbegin");
	});

	it("lineBoundaries가 아예 없으면 null", () => {
		const result = call([makeToken({ flags: TOKEN_FLAGS_LINE_START, containerIndex: 1 })], 0, [], [], 0);
...
describe("Case C — filled이 container 경계에서 시작", () => {
	it("gap lineBoundary(중간 container)가 있으면 사용", () => {
		...
		const result = call([filledPrev, filledStart], 1, [emptyPrev, emptyNext], [lb0, lb1, lb2, lb3], 1);
	...
	it("gap lineBoundary 없이 prev·next container만 있으면 Case D로 fallthrough", () => {
		...
		const result = call([filledPrev, filledStart], 1, [emptyPrev, emptyNext], [lb1, lb2], 1);
	...
	it("filled가 <br>로 인한 LINE_START(같은 container) → Case C skip → Case D", () => {
		...
		const result = call([filledPrev, filledStart], 1, [emptyPrev, emptyNext], [lb2], 1);
	...
	it("filledStart=0 (문서 맨 앞)은 container 경계로 취급 — contentPrev 없으면 Case D fallback", () => {
		...
		const result = call([filledStart], 0, [emptyNext], [], 0);
...
describe("structural 토큰 처리", () => {
	it("filledStart가 STRUCTURAL_OPEN이면 첫 content 토큰으로 skip", () => {
		...
		const structOpen = makeToken({
			flags: TOKEN_FLAGS_STRUCTURAL_OPEN,
			startNode: tdEl,
			endNode: tdEl,
			containerIndex: 3,
			lineNumber: 2,
		});
		const content = makeToken({
			flags: TOKEN_FLAGS_LINE_START,
			startNode: textNode,
			endNode: textNode,
			containerIndex: 3,
			lineNumber: 2,
		...
		const result = call(
			[filledPrev, structOpen, content],
			1, // filledStart=1 (STRUCTURAL_OPEN)
			[emptyPrev],
			[],
			1,
		);
