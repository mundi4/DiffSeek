...
export async function runHistogramDiff(
	...
	const {
		...
		buffer: _lhsTextBuffer,
		offsets: _lhsOffsets,
		flags: _lhsFlags,
		resultBuffer: _lhsResultBuffer,
	} = lhsInput;
	const {
		tokenCount: _rhsTokenCount,
		buffer: _rhsTextBuffer,
		offsets: _rhsOffsets,
		flags: _rhsFlags,
		resultBuffer: _rhsResultBuffer,
	} = rhsInput;
	...
	async function diffCore(lhsLower: number, lhsUpper: number, rhsLower: number, rhsUpper: number): Promise<void> {
		...
		if (anchor) {
			...
			if (anchor.lhsEnd - anchor.lhsStart !== anchor.rhsEnd - anchor.rhsStart) {
				...
				throw new Error(
					`Anchor length mismatch: lhs length ${anchor.lhsEnd - anchor.lhsStart}, rhs length ${anchor.rhsEnd - anchor.rhsStart}`,
				);
			}

			// TODO _ignoreWhitespaces 옵션이 켜진 경우에만 consumeCommonEdges(..., 2) 호출
			let tmpLhsLower, tmpLhsUpper, tmpRhsLower, tmpRhsUpper;
			if (_ignoreWhitespaces) {
				[tmpLhsLower, tmpLhsUpper, tmpRhsLower, tmpRhsUpper] = consumeCommonEdges(
					lhsLower,
					anchor.lhsStart,
					rhsLower,
					anchor.rhsStart,
				...
				tmpLhsLower = lhsLower;
				tmpLhsUpper = anchor.lhsStart;
				tmpRhsLower = rhsLower;
				tmpRhsUpper = anchor.rhsStart;
			}
			...
			// TODO _ignoreWhitespaces 옵션이 켜진 경우에만 consumeCommonEdges(..., 1) 호출
			if (_ignoreWhitespaces) {
				[tmpLhsLower, tmpLhsUpper, tmpRhsLower, tmpRhsUpper] = consumeCommonEdges(
					anchor.lhsEnd,
					lhsUpper,
				...
				tmpLhsLower = anchor.lhsEnd;
				tmpLhsUpper = lhsUpper;
				tmpRhsLower = anchor.rhsEnd;
				tmpRhsUpper = rhsUpper;
			}
			...
			if (_ignoreWhitespaces) {
				...
				[lhsLower, lhsUpper, rhsLower, rhsUpper] = consumeCommonEdges(
					lhsLower,
					lhsUpper,
					rhsLower,
					rhsUpper,
	...
	function fallbackGreedyConsume(lhsLo: number, lhsHi: number, rhsLo: number, rhsHi: number) {
		let lCur = lhsLo;
		let rCur = rhsLo;
	...
	function findFirstCrossMatch(
		...
		for (let d = 0; d <= maxD; d++) {
			...
			for (let i = iStart; i <= iEnd; i++) {
				const j = d - i;
				const r = matchPrefixTokens(lhsInput, rhsInput, lhsLo + i, lhsHi, rhsLo + j, rhsHi);
				if (r !== null) {
	...
	async function findAnchor(
			...
			for (let i = 0; i < numCandidates; i++) {
				...
				if ((_lhsFlags[rep] & TOKEN_TYPE_MASK) === TOKEN_FLAGS_TYPE_STRUCTURAL) {
					...
					if (isStructuralOnly) {
						...
						const sop =
							_structuralOnlyMultipliers[mIdx] * (_structuralLevelBonuses[maxStructuralLevel] ?? 1);
		...
		for (let i = 1; i <= intervalCount; i++) {
			let lastLo = i - 1;
			const currentLCP = i === intervalCount ? 0 : localLcp[i];
