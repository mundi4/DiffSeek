...
import {
	...
	DIRTY_DIFF_HIGHLIGHT_OFFSCREEN,
	DIRTY_GEOMETRY,
	DIRTY_MINIMAP,
	DIRTY_RESIZE,
	DIRTY_SCROLL,
...
export class EditorRegion {
	...
	setHighlightedDiffIndex(diffIndex: number | null) {
		...
		let shouldShow =
			diffIndex !== null && (!this.diffGeometries[diffIndex] || this.visibleDiffIndices.has(diffIndex));
		...
		for (const diffIndex of this.#sortedDiffIndices) {
			...
			if (geometry.rects === null) {
				...
				if (rangeRects.length > 0) {
					...
					if (import.meta.env.DEV) {
						console.warn(
							`[EditorRegion:${this.editor.name}] extractRectsFromRange returned empty for diff ${diffIndex}`,
						);
					}
