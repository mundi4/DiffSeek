...
function tokenizeByLine(input: string): Token[] {
	...
	for (let i = inputPos; i < inputEnd; i++) {
		...
		if (char !== "\n") {
			...
			if (currentStart !== -1) {
				tokens.push({
					text: input.substring(currentStart, currentEnd).replace(/ +/g, " ").trim(),
					pos: currentStart,
					len: i - currentStart,
					lineNum: lineNum,
					flags: flags,
...
async function runLcsDiff(ctx: WorkContext): Promise<PostProcessResult> {
...
async function runMyersDiff(ctx: WorkContext): Promise<PostProcessResult> {
...
async function runHistogramDiff(ctx: WorkContext): Promise<PostProcessResult> {
	const lhsTokens = tokenize(ctx.leftText, ctx.options.tokenization, false);
...
const findBestHistogramAnchor: FindAnchorFunc = function (
	...
	const LENGTH_BIAS_FACTOR = 0.7;
	const UNIQUE_BONUS = 0.5;
	const LINE_START_BONUS = 0.85;
	const LINE_END_BONUS = 0.9;
	//const FULL_LINE_BONUS = 0.85; n그램을 사용시 여러단어가 매치되는 경우 오히려 마지막 단어가 다음 줄로 넘어가서 보너스를 못 받을 수가 있다
	const SECTION_HEADING_BONUS = 0.75;
...
function postProcess(ctx: WorkContext, rawEntries: DiffEntry[], leftTokens: Token[], rightTokens: Token[]) {
