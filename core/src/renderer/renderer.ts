...
export class Renderer {
	...
	updateLayout() {
		...
		if (this.scaledWidth !== scaledWidth || this.scaledHeight !== scaledHeight) {
			...
			this.nextRenderFlags = RENDER_GENERAL | RENDER_REGION_MASK | (RENDER_REGION_MASK << RENDER_REGION_SHIFT);
		}
	...
	render(time: number) {
		...
		if (leftRegionFlags) {
			...
			if (leftRegionFlags & RENDER_DIFF_LAYER) {
				leftDiffVisibilityChangeEntries = this.updateVisibleDiffIndices(
					this.visibleDiffIndices.left,
					this.leftRegion.visibleDiffIndices,
				);
			}
		}
		if (rightRegionFlags) {
			...
			if (rightRegionFlags & RENDER_DIFF_LAYER) {
				rightDiffVisibilityChangeEntries = this.updateVisibleDiffIndices(
					this.visibleDiffIndices.right,
					this.rightRegion.visibleDiffIndices,
				);
			}
		}
