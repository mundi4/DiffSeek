...
function handleDiffResult(reqId: number, result: DiffResult) {
	self.postMessage(
		...
		[result.leftResultBuffer.buffer, result.rightResultBuffer.buffer],
	);
}
...
async function runDiffJob(workItem: WorkItem, abortSignal: AbortSignal): Promise<DiffResult> {
	...
	if (leftTokenCount === 0 && rightTokenCount === 0) {
		...
		const { input: lhsInput, lineCount: lhsLineCount } = buildDiffInput(
			workItem.leftWholeText,
			workItem.leftTokenBuffer,
			workItem.diffOptions,
		);
		const { input: rhsInput, lineCount: rhsLineCount } = buildDiffInput(
			workItem.rightWholeText,
			workItem.rightTokenBuffer,
			workItem.diffOptions,
		);
		...
		const usePatience = shouldUsePatience(lhsInput, rhsInput, workItem.diffOptions, lhsLineCount, rhsLineCount);

		if (usePatience) {
			const anchors = buildPatienceAnchors(lhsInput, rhsInput, lhsLineCount, rhsLineCount, workItem.diffOptions);

			await processSegmentsWithAnchors(ctx, lhsInput, rhsInput, anchors);
		} else {
