...
const ABORT_REASON_CANCELLED = "aborted";
const VOID_ELEMENTS = {
  AREA: true,
  BASE: true,
  BR: true,
...
const DIFF_TAG_NAME = "DS-DIFF";
const MANUAL_ANCHOR_TAG_NAME = "DS-MANUAL-ANCHOR";
const ANCHOR_TAG_NAME = "DS-ANCHOR";
const ANCHOR_CLASS_NAME = "ds-anchor";
class Scheduler {
  signal;
  iterationCount = 0;
  lastYieldTime = 0;
  ...
  constructor(options = {}) {
    ...
    this.yieldInterval = this.#findNearestMask(options.yieldInterval ?? 127);
   ...
   */
  #findNearestMask(value) {
    if (value <= 0) return 127;
    if ((value + 1 & value) === 0) {
      return value;
    }
    const highestBit = Math.floor(Math.log2(value));
    const lower = (1 << highestBit) - 1;
    const upper = (1 << highestBit + 1) - 1;
...
function paragraphizePlainText(plaintext, trimLines = false) {
...
function copyAllowedStyles(from, to, allowed) {
  ...
  for (const k in allowed) {
    const v = from.getPropertyValue(k);
    if (v) to.setProperty(k, v);
  }
}
  ...
  async function traverse(node) {
    ...
    if (node.nodeType === 1) {
      ...
      if (node.nodeName === "DIV") {
        if (el.className === "aspNetHidden" || el.className === "pak_aside clear" || el.className === "pak_tab_menu" || el.className === "listBtn" || el.className === "ManualEvalWrap") return null;
    ...
    if (nodeName === "DIV" && node.dataset.origin === "word-textbox") {
      container.classList.add("word-textbox");
    }
...
const isReddish = /* @__PURE__ */ (() => {
  let ctx = null;
  const reddishCache = /* @__PURE__ */ new Map([["red", true], ["#ff0000", true], ["#e60000", true], ["#c00000", true], ["rgb(255,0,0)", true], ["rgb(230,0,0)", true], ["#000000", false], ["#333333", false], ["#ffffff", false], ["black", false], ["blue", false], ["white", false], ["window", false], ["windowtext", false]]);
...
var TokenFlags = /* @__PURE__ */ ((TokenFlags2) => {
  ...
  TokenFlags2[TokenFlags2["WORD_LIKE"] = 1] = "WORD_LIKE";
  TokenFlags2[TokenFlags2["PUNCTUATION"] = 2] = "PUNCTUATION";
  TokenFlags2[TokenFlags2["LINE_START"] = 4] = "LINE_START";
  TokenFlags2[TokenFlags2["LINE_END"] = 8] = "LINE_END";
  TokenFlags2[TokenFlags2["SECTION_HEADING_TYPE1"] = 16] = "SECTION_HEADING_TYPE1";
  TokenFlags2[TokenFlags2["SECTION_HEADING_TYPE2"] = 32] = "SECTION_HEADING_TYPE2";
  TokenFlags2[TokenFlags2["SECTION_HEADING_TYPE3"] = 64] = "SECTION_HEADING_TYPE3";
  TokenFlags2[TokenFlags2["SECTION_HEADING_TYPE4"] = 128] = "SECTION_HEADING_TYPE4";
  TokenFlags2[TokenFlags2["SECTION_HEADING_TYPE5"] = 256] = "SECTION_HEADING_TYPE5";
  TokenFlags2[TokenFlags2["SECTION_HEADING_TYPE6"] = 512] = "SECTION_HEADING_TYPE6";
  TokenFlags2[TokenFlags2["LAW_ARTICLE"] = 1024] = "LAW_ARTICLE";
  TokenFlags2[TokenFlags2["WILDCARD"] = 2048] = "WILDCARD";
  TokenFlags2[TokenFlags2["MANUAL_ANCHOR"] = 4096] = "MANUAL_ANCHOR";
  TokenFlags2[TokenFlags2["IMAGE"] = 8192] = "IMAGE";
  TokenFlags2[TokenFlags2["NO_JOIN_NEXT"] = 16384] = "NO_JOIN_NEXT";
  TokenFlags2[TokenFlags2["NO_JOIN_PREV"] = 32768] = "NO_JOIN_PREV";
  return TokenFlags2;
})(TokenFlags || {});
...
const OPTIONAL_SPACE_MARKER = "§";
function createTrie() {
  return {
    next: {}
  };
}
function insertIntoTrie(trie, word, text, flags) {
  let node = trie;
...
function matchTrie(trie, segments, segmentIndex) {
  let node = trie;
...
function tryMatchNumbering(text) {
  ...
  if (firstChar === "제") {
    ...
    charIndex++;
    let charCode;
    if (!(charIndex === text.length || WS_TABLE[charCode = text.charCodeAt(charIndex)] || charCode === 46 || charCode === 40)) {
      return null;
    }
    return {
    ...
    charIndex++;
    skipWhitespace();
    const number2 = readNumberOrHangulOrder();
    if (!number2) return null;
    skipWhitespace();
    if (text[charIndex] !== ")") {
      return null;
    ...
    return {
      ...
      number: number2,
      length: charIndex,
      type: hangulOrderSet[number2] ? TokenFlags.SECTION_HEADING_TYPE4 : TokenFlags.SECTION_HEADING_TYPE3
    };
  }
...
async function tokenize(root, options = {}) {
  const tokens = [];
  ...
  let lineStartWhich = null;
  let lineStartWhere = null;
  ...
  await walkDOMAsync(root);
  if (tokens.length > 0) {
    tokens[tokens.length - 1].flags |= TokenFlags.LINE_END;
  }
  scheduler2.throwIfAborted();
  ...
  async function walkDOMAsync(element) {
  ...
  function markNewLine(which, where, immediate = false) {
    isLineStart = true;
  ...
  function handleTextNode(text) {
    ...
    for (let i = 0; i < raw.length; i++) {
      const ch0 = raw[i];
      const mapped = normalizedCharMap[ch0];
      if (mapped !== void 0) {
        if (last !== i) {
  ...
  function flushTextBuf() {
    ...
    for (let i2 = 0; i2 < tokenText.length; i2++) {
      const code = tokenText.charCodeAt(i2);
      const ws = WS_TABLE[code];
      if (!ws) {
        collapsable = false;
        if (firstNonSpaceIndex === -1) {
          firstNonSpaceIndex = i2;
          break;
        }
      } else {
        if (ws === 2) {
    ...
    if (firstNonSpaceIndex !== -1) {
      for (let i2 = tokenText.length - 1; i2 >= 0; i2--) {
        const code = tokenText.charCodeAt(i2);
        if (!WS_TABLE[code]) {
          lastNonSpaceIndex = i2;
          break;
        }
      }
    }
    ...
    if (isLineStart) {
      ...
      if (numbering) {
        ...
        addToken(TokenType.TEXT, numbering.wholeText, TokenFlags.LINE_START | numbering.type, textNodeBuf[startNodeIndex], startCharIndex, textNodeBuf[endNodeIndex], endCharIndex + 1);
    ...
    while (i < segments.length) {
      ...
      if (currentSegment.isWordLike) {
        if (segmentCount > 0) {
          if (addSegmentedToken(segments, segmentStart, segmentCount, TokenFlags.WORD_LIKE)) ;
          segmentCount = 0;
        }
        addSegmentedToken(segments, i, 1, 0);
        i++;
        continue;
      }
      if (segmentCount === 0) {
    ...
    if (segmentCount > 0) {
      if (addSegmentedToken(segments, segmentStart, segmentCount, 0)) ;
    }
    resetTextBuf();
    return true;
  }
  function resetTextBuf() {
    textNodeBuf.length = 0;
    currentTokenText = "";
    mapNodeIndex.length = 0;
    mapCharIndex.length = 0;
  }
  function addSegmentedToken(segments, startIdx, count, flags = 0, text) {
    ...
    if (startNodeIndex === void 0 || endNodeIndex === void 0) {
      return false;
    }
    ...
    addToken(TokenType.TEXT, text, flags, startNode, mapCharIndex[startSeg.startOffset], endNode, mapCharIndex[endSeg.endOffset - 1] + 1);
  }
  function addImageToken(img) {
    return addToken(TokenType.IMAGE, img.src || "image", TokenFlags.IMAGE, img, 0, img, 0);
  }
  function addToken(type, text, flags, startNode, startOffset, endNode, endOffset) {
...
function SetStartEndFromTokenRange(range, tokenRange, ensureConnected = true) {
...
function createRangeFromTokenRange(tokenRange, ensureConnected = true) {
  const range = document.createRange();
  SetStartEndFromTokenRange(range, tokenRange, ensureConnected);
  return range;
}
function setStartFromTokenRange(range, tokenRange, ensureConnected = true) {
  if (tokenRange.type === TokenType.TEXT) {
    if (ensureConnected && !tokenRange.startNode?.isConnected) {
...
function setEndFromTokenRange(range, tokenRange, ensureConnected = true) {
  if (tokenRange.type === TokenType.TEXT) {
...
function setRangeBoundary(range, tokenRange, boundary, position, ensureConnected = true) {
  if (tokenRange.type === TokenType.TEXT) {
    ...
    const fn = boundary === "start" ? position === "before" ? range.setStartBefore : range.setStartAfter : position === "before" ? range.setEndBefore : range.setEndAfter;
    fn.call(range, tokenRange.startNode);
  }
}
function setStartAfterToken(range, tokenRange, ensureConnected = true) {
  setRangeBoundary(range, tokenRange, "start", "after", ensureConnected);
}
function setEndBeforeToken(range, tokenRange, ensureConnected = true) {
...
class Editor {
  ...
  constructor(name, settings = {}) {
    ...
    this.contentElement.appendChild(INITIAL_EDITOR_HTML.cloneNode(true));
  ...
  set isSyncMode(value) {
    if (this.#isSyncMode === value) {
      return;
    }
    this.#isSyncMode = value;
  ...
  #onKeyDown(e) {
    if (e.ctrlKey && (e.key === "ArrowUp" || e.key === "ArrowDown")) {
      if (this.rootElement) {
        ...
        const delta = (e.key === "ArrowUp" ? -this.#settings.lineHeight : this.#settings.lineHeight) * this.#settings.altArrowScrollLines * fontSize;
  ...
  async #tokenize() {
    ...
    try {
      const {
        ...
        lineStartPoints,
        elapsed
      } = await tokenize(this.contentElement, {
    ...
    if (r.collapsed) {
      let i = n;
      {
        ...
        while (lo <= hi) {
          const mid = lo + hi >> 1;
          const t2 = tokens[mid];
          SetStartEndFromTokenRange(tokenRange, t2);
  ...
  forceReflow() {
    ...
    void this.rootElement.offsetHeight;
  }
  ...
  ensureInsertableRange(range, forBlock, allowSplitting = false) {
    ...
    if (forBlock) {
      ...
      let adjusted = false;
      while (container !== this.contentElement && container.nodeName !== "TD" && container.nodeName !== "TH" && (container.nodeName === "P" || !BLOCK_ELEMENTS[container.nodeName])) {
  ...
  #lastMarkerEl = null;
  getOrInsertDiffElement(tokenIndex, hint) {
    let which = null;
    let where = null;
    if (tokenIndex === 0) {
      ...
      let a = prevToken.endNode;
      let b = nextToken.startNode;
      let da = 0, db = 0;
    ...
    if (!el || el.nodeName !== DIFF_TAG_NAME) {
      ...
      if (where === "afterend") {
        ...
        which.insertAdjacentElement("afterbegin", el);
      }
    }
    return el;
  }
  getOrInsertAnchorElement(tokenIndex) {
    let where = null;
    let which = null;
    if (this.tokens.length === 0) {
      ...
      ({
        which,
        where
      } = this.lineStartPoints[token.lineNumber - 1]);
    }
    if (!where || !which) {
      return null;
    }
    ...
    if (where === "afterend") {
      ...
      anchorEl = which.previousSibling;
      if (!anchorEl || anchorEl.nodeName !== ANCHOR_TAG_NAME) {
        anchorEl = document.createElement(ANCHOR_TAG_NAME);
        which.parentElement.insertBefore(anchorEl, which);
      }
    } else if (where === "afterbegin") {
      anchorEl = which.firstChild;
      if (!anchorEl || anchorEl.nodeName !== ANCHOR_TAG_NAME) {
        anchorEl = document.createElement(ANCHOR_TAG_NAME);
        which.insertAdjacentElement("afterbegin", anchorEl);
      }
    } else if (where === "beforeend") {
      anchorEl = which.lastChild;
      if (!anchorEl || anchorEl.nodeName !== ANCHOR_TAG_NAME) {
        anchorEl = document.createElement(ANCHOR_TAG_NAME);
        which.insertAdjacentElement("beforeend", anchorEl);
      }
    }
  ...
  //     } else {
  //         const token = this.tokens[tokenIndex];
  //         const lineStartPoint = this.lineStartPoints[token.lineNumber - 1];
  //         if (lineStartPoint) {
  //         }
  //         console.log("lineStartPoint for token", tokenIndex, "line", token.lineNumber, "is", lineStartPoint);
  //         const root = this.contentElement;
  //         let node: Node | null = token.startNode;
  //         let skipChildren = true;
  //         let stopNode: Node | null = null;
  //         if (tokenIndex > 0) {
  //             stopNode = this.tokens[tokenIndex - 1].endNode;
  //         }
  //         while (node) {
  //             if (node === stopNode) {
  //                 return null;
  //             }
  //             if (node.nodeName === "BR" || node.nodeName === "HR" || node.nodeName === MANUAL_ANCHOR_TAG_NAME) {
  //                 // 이 다음에
  //                 where = "afterend";
  //                 which = node;
  //                 break;
  //             }
  //             if (CONTAINER_TAGS[node.nodeName] || node === root) {
  //                 // 이 안에
  //                 where = "afterbegin";
  //                 which = node;
  //                 break;
  //             }
  //             // THEAD, TBODY, TFOOT는 sanitize 단계에서 제거됨.
  //             // TR 같은건 만날 일이 없어야 함.
  //             // 삽입을 항상 토큰의 앞에. 즉 startNode의 앞.
  //             // startNode는 text노드이거나 img 같은 실제 visual을 가진 요소이기 때문에 TR을 만나기 전에 반드시 TD/TH를 만나게 되어 있음.
  ...
  //                 break;
  //             }
  //             // <span><em>hello</em></span>. 토큰index는 hello(text노드)인 경우
...
class DiffProcessor {
  #leftEditor;
  #rightEditor;
  #markerElements = /* @__PURE__ */ new Set();
  ...
  async process({
    ...
    this.#markerElements = /* @__PURE__ */ new Set();
    const leftTokens = this.#leftEditor.tokens;
    const rightTokens = this.#rightEditor.tokens;
    const diffs = [];
    const anchorPairs = [];
    const leftEntries = new Array(leftTokens.length);
    const rightEntries = new Array(rightTokens.length);
    if (entries.length > 0) {
      ...
      for (const entry of entries) {
        ...
        if (entry.type === 0) {
          ...
          if (leftTokenCount > 0 && rightTokenCount > 0) {
            ...
            let markerPositionWhere = null;
            let markerPositionNode = null;
            if (filledToken.flags & TokenFlags.LINE_START) {
              ...
              if (emptySpan.start === 0) {
                ...
                markerPositionNode = emptyRoot;
              } else if (emptyPrevToken.endNode !== emptyToken.startNode) {
          ...
          diffs.push({
            ...
            rightRange,
            leftSpan: {
              start: entry.left.start,
              end: entry.left.end
            },
            rightSpan: {
              start: entry.right.start,
              end: entry.right.end
            },
  ...
  #cleanupUnsuedMarkers(previouslyUsedMarkers) {
  ...
  #addAnchorPair(anchorPairs, leftToken, leftEl, rightToken, rightEl, diffIndex) {
    if (!leftEl) {
  ...
  #getOrCreateDiffElement(where, node) {
    if (!where || !node) {
      return null;
    }
    let diffEl = null;
    if (where === "afterend") {
      diffEl = node.nextSibling;
    } else if (where === "afterbegin") {
      diffEl = node.firstChild;
    } else if (where === "beforeend") {
      diffEl = node.lastChild;
    } else {
    ...
    if (!diffEl || diffEl.nodeName !== DIFF_TAG_NAME) {
      ...
      if (where === "afterend") {
        ...
        node.insertAdjacentElement(where, diffEl);
      }
    }
...
function getRangeFromElement(el) {
  if (!el) return null;
  const range = document.createRange();
  range.selectNode(el);
  return range;
}
function activateAnchorElement(el, anchorIndex, diffIndex) {
...
function deactivateAnchorElement(el) {
...
const defaultDiffOptions = {
  ...
  // ─────────────────────────────
  useLengthBonus: true,
  lengthBonusMultiplier: 1,
  maxLengthPerGramForBonus: 5,
  // ─────────────────────────────
  // Line start bonus
  // ─────────────────────────────
  useLineStartBonus: true,
  lineStartBonusMultiplier: 1,
  // ─────────────────────────────
  // Uniqueness bonus
  // ─────────────────────────────
  useUniqueBonus: true,
  uniqueBonusMultiplier: 1,
  // ─────────────────────────────
...
var RenderFlags = /* @__PURE__ */ ((RenderFlags2) => {
  RenderFlags2[RenderFlags2["NONE"] = 0] = "NONE";
  RenderFlags2[RenderFlags2["LAYOUT"] = 1] = "LAYOUT";
  RenderFlags2[RenderFlags2["HIT_TEST"] = 2] = "HIT_TEST";
  RenderFlags2[RenderFlags2["GEOMETRY"] = 32] = "GEOMETRY";
  RenderFlags2[RenderFlags2["SCROLL"] = 64] = "SCROLL";
  RenderFlags2[RenderFlags2["RESIZE"] = 128] = "RESIZE";
  RenderFlags2[RenderFlags2["SELECTION_HIGHLIGHT"] = 256] = "SELECTION_HIGHLIGHT";
  RenderFlags2[RenderFlags2["DIFF_HIGHLIGHT"] = 512] = "DIFF_HIGHLIGHT";
  RenderFlags2[RenderFlags2["MINIMAP"] = 160] = "MINIMAP";
  RenderFlags2[RenderFlags2["DIFF_LAYER"] = 224] = "DIFF_LAYER";
  RenderFlags2[RenderFlags2["HIGHLIGHT_LAYER"] = 992] = "HIGHLIGHT_LAYER";
  RenderFlags2[RenderFlags2["GENERAL_MASK"] = 3] = "GENERAL_MASK";
  RenderFlags2[RenderFlags2["REGION_MASK"] = 992] = "REGION_MASK";
  return RenderFlags2;
})(RenderFlags || {});
...
class EditorRegion {
  ...
  updateLayout() {
    let {
      ...
      width,
      height
    } = this.editor.rootElement.getBoundingClientRect();
  ...
  prepare(dirtyFlags) {
    ...
    for (let diffIndex = 0; diffIndex < diffs.length; diffIndex++) {
      ...
      if (geometry.rects === null) {
        const rangeRects = this.#extractRectsFromRange(diffs[diffIndex].range, offsetLeft, offsetTop, diffExpandX, diffExpandY, diffs[diffIndex].empty);
  ...
  #extractRectsFromRange(range, offsetLeft, offsetTop, expandX = 0, expandY = 0, emptyDiff = false) {
    ...
    let startNode;
    if (emptyDiff && range.startContainer.nodeType === 1 && range.startContainer === range.endContainer && range.startOffset + 1 === range.endOffset) {
      const diffEl = range.startContainer.childNodes[range.startOffset];
...
class Renderer {
  ...
  #nextRenderFlags = RenderFlags.NONE;
  #mouseX = -1;
  #mouseY = -1;
  #syncMode = false;
  #guideLineY = -1;
  ...
  setOptions(newOptions) {
    deepMerge$1(this.options, newOptions);
  }
  getOptions() {
    return this.options;
  }
  ...
  hitTest(x, y) {
    ...
    if (x < 0 || y < 0) ;
      ...
      if (this.#syncMode && this.#guideLineY !== guideLineY) {
...
function resolveMatchingSpanPair(diffContext, side, sourceSpan) {
...
class DiffseekEngine {
  ...
  alignAnchorsAbortController = null;
  contentChanging = {
    left: false,
    right: false
  };
  ...
  diffState = null;
  syncModeChanged = createEvent();
  diffContextChanged = createEvent();
  statusChanged = createEvent();
  diffVisibilityChanged = createEvent();
  _eventRegistry = {
  ...
  setDiffOptions(newOptions) {
    if (newOptions) {
      ...
      this.diffOptions = getDefaultDiffOptions();
    }
  }
  getDiffOptions() {
    return structuredClone(this.diffOptions);
  }
  //
  // #region Callbacks
  //
  ...
  #handleSelectionChange() {
    ...
    if (selection) {
      ...
      if (selection.sourceEditor === "left") {
        this.leftEditor, targetEditor = this.rightEditor;
        selection.left, targetSpan = selection.right;
        selection.sourceRange;
      } else {
        this.rightEditor, targetEditor = this.leftEditor;
        selection.right, targetSpan = selection.left;
        selection.sourceRange;
      }
    ...
    if (this._syncMode) {
      ...
      if (Math.abs(scrollTop - otherScrollTop) >= 1) {
        if (!this.programmaticScrollInProgress) {
          ...
          otherEditor.scrollTo(scrollTop, {
            behavior: "instant"
          });
          this.programmaticScrollInProgress--;
        }
  ...
  set syncMode(value) {
    ...
    this.syncModeChanged.emit({
      syncMode: value
    });
  }
  scrollToDiff(diffIndex, side, options) {
    const rect = this.renderer.getDiffRect(side, diffIndex);
    if (rect) {
  ...
  async #startDiffWorkflow() {
    ...
    try {
      ...
      const diffOptions = this.diffOptions;
      const workerResult = await this.diffWorker.run({
        leftTokens,
        rightTokens,
        options: diffOptions,
        abortSignal: scheduler2.signal,
...
function App() {
  ...
  if ($[0] === /* @__PURE__ */ Symbol.for("react.memo_cache_sentinel")) {
    t0 = () => {
      ...
      engine.on("diffContextChanged", _temp);
      engine.on("statusChanged", _temp2);
      engine.on("diffVisibilityChanged", _temp3);
      engine.on("syncModeChanged", _temp4);
    };
    t1 = [];
    $[0] = t0;
    $[1] = t1;
  ...
  if ($[2] === /* @__PURE__ */ Symbol.for("react.memo_cache_sentinel")) {
    t2 = /* @__PURE__ */ jsxRuntimeExports.jsx(jsxRuntimeExports.Fragment, { children: /* @__PURE__ */ jsxRuntimeExports.jsx("main", { id: "diffseek-host", ref: hostRef }) });
    $[2] = t2;
  } else {
    t2 = $[2];
  }
...
function _temp4(syncMode) {
  console.log("Sync mode changed:", syncMode);
}
function _temp3(visibleDiffs) {
  console.log("Visible diffs changed:", visibleDiffs);
}
function _temp2(diffState) {
  console.log("Diff state changed:", diffState);
}
function _temp(diffContext) {
...
clientExports.createRoot(document.getElementById("app")).render(/* @__PURE__ */ jsxRuntimeExports.jsx(reactExports.StrictMode, { children: /* @__PURE__ */ jsxRuntimeExports.jsx(MantineProvider, { children: /* @__PURE__ */ jsxRuntimeExports.jsx(App, {}) }) }));
