import type { DiffWorkerMessage, DiffWorkerRequest } from "./diff-worker";
...
export type OnDiffCompleteCallback = (result: DiffResult) => void;
...
export function initializeDiffWorker(onComplete: OnDiffCompleteCallback) {
	...
	worker.onmessage = (e: MessageEvent<DiffWorkerMessage>) => {
		const data = e.data;
		if (data.type === "diff") {
