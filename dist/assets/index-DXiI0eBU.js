...
var h, p, m = "undefined" != typeof window && !!window.__PREACT_SIGNALS_DEVTOOLS__, g = [];
E(function() {
  h = this.N;
})();
function b(i2, t2) {
  l$2[i2] = t2.bind(null, l$2[i2] || function() {
  });
}
function y(i2) {
  if (p) p();
...
Object.defineProperties(u.prototype, { constructor: { configurable: true, value: void 0 }, type: { configurable: true, value: w }, props: { configurable: true, get: function() {
  return { data: this };
} }, __b: { configurable: true, value: 1 } });
b("__b", function(i2, n2) {
...
b("__r", function(i2, n2) {
  if (n2.type !== k$2) {
    y();
    var t2, f2 = n2.__c;
    if (f2) {
      f2.__$f &= -2;
      if (void 0 === (t2 = f2.__$u)) f2.__$u = t2 = (function(i3, n3) {
        var t3;
        E(function() {
          t3 = this;
        }, { name: n3 });
...
b("__e", function(i2, n2, t2, r2) {
  y();
  i2(n2, t2, r2);
});
b("diffed", function(i2, n2) {
  y();
  var t2;
...
function F() {
  r(function() {
    var i2;
    while (i2 = g.shift()) h.call(i2);
  });
}
function M() {
  if (1 === g.push(this)) (l$2.requestAnimationFrame || A)(F);
}
...
class Scheduler {
  ...
  async #doYield() {
    const g2 = globalThis;
    if (g2.scheduler?.yield) {
      await g2.scheduler.yield();
    } else {
...
function sliceFragment(html) {
  const s2 = html.indexOf(START_TAG);
  if (s2 < 0) return html;
  const e2 = html.lastIndexOf(END_TAG);
  return e2 >= 0 ? html.slice(s2 + START_TAG.length, e2) : html.slice(s2 + START_TAG.length);
}
const _EMPTY_LINE = (() => {
...
function copyAllowedStyles(from, to, allowed) {
  if (!allowed) return;
  for (const k2 in allowed) {
    const v2 = from.getPropertyValue(k2);
    if (v2) to.setProperty(k2, v2);
  }
}
function normalizeFont(raw) {
  if (!raw) return null;
  let s2 = raw.split(",")[0].trim();
...
const isReddish = /* @__PURE__ */ (() => {
  ...
  function getRGB(color) {
    ...
    if (hex6) {
      const n2 = parseInt(hex6[1], 16);
      return [n2 >> 16 & 255, n2 >> 8 & 255, n2 & 255];
    }
    const hex3 = /^#([0-9a-f]{3})$/i.exec(color);
    if (hex3) {
      const [r2, g2, b2] = hex3[1].split("").map((c2) => parseInt(c2 + c2, 16));
      return [r2, g2, b2];
    }
    const rgb = /^rgba?\(([^)]+)\)$/i.exec(color);
    if (rgb) {
      const parts = rgb[1].split(",").map((s2) => parseInt(s2.trim(), 10));
...
const hangulOrderSet = (() => {
  ...
  for (let i2 = 0; i2 < HANGUL_ORDER.length; i2++) set[HANGUL_ORDER[i2]] = true;
  return set;
})();
function tryMatchNumbering(text) {
  ...
  const readNumber = () => {
    ...
    while (charIndex < text.length) {
      const c2 = text.charCodeAt(charIndex);
...
async function tokenize(root, options = {}) {
  ...
  function handleTextNode(text) {
    ...
    for (let i2 = 0; i2 < raw.length; i2++) {
      ...
      if (mapped !== void 0) {
        if (last !== i2) {
          out += raw.slice(last, i2);
        }
        for (let j2 = 0; j2 < mapped.length; j2++) {
          mapNodeIndex.push(nodeIndex);
          mapCharIndex.push(i2);
        }
        out += mapped;
        last = i2 + 1;
      } else {
  ...
  function flushTextBuf() {
    ...
    for (let i22 = 0; i22 < tokenText.length; i22++) {
      const code = tokenText.charCodeAt(i22);
      const ws = WS_TABLE[code];
      if (!ws) {
        collapsable = false;
        if (firstNonSpaceIndex === -1) {
          firstNonSpaceIndex = i22;
          break;
        }
      } else {
        if (ws === 2) {
    ...
    if (firstNonSpaceIndex !== -1) {
      for (let i22 = tokenText.length - 1; i22 >= 0; i22--) {
        const code = tokenText.charCodeAt(i22);
        if (!WS_TABLE[code]) {
          lastNonSpaceIndex = i22;
          break;
        }
      }
    }
    ...
    while (i2 < segments.length) {
      const currentSegment = segments[i2];
      if (WS_TABLE[currentSegment.text.charCodeAt(0)]) {
        flushSegments();
        i2++;
        continue;
      }
      const match = matchTrie(wildcardTrie, segments, i2);
      if (match) {
        flushSegments();
        addSegmentedToken(segments, i2, match.count, match.flags, match.word);
        i2 += match.count;
        continue;
      }
      if (currentSegment.isWordLike) {
        ...
        addSegmentedToken(segments, i2, 1, 0);
        i2++;
        continue;
      }
      if (segmentCount === 0) {
  ...
  function addSegmentedToken(segments, startIdx, count, flags = 0, text) {
    ...
    if (text === void 0) {
      text = "";
      for (let i2 = startIdx; i2 <= lastIdx; i2++) {
        text += segments[i2].text;
      }
    }
...
class Editor {
  ...
  constructor(name, settings = {}) {
    ...
    this.contentElement.addEventListener("copy", (e2) => this.#onCopy(e2));
    this.contentElement.addEventListener("paste", (e2) => this.#onPaste(e2));
    this.contentElement.addEventListener("input", () => this.#handleContentChangedInternal());
    this.contentElement.addEventListener("click", (e2) => {
      this.#callbacks.click?.(this, e2);
    });
    this.contentElement.addEventListener("keydown", (e2) => this.#onKeyDown(e2));
  ...
  #onContainerMouseMove = (e2) => {
    this.#callbacks.mouseMove?.(this, e2);
  };
  #onContainerMouseLeave = (e2) => {
    this.#callbacks.mouseLeave?.(this, e2);
  };
  get isSyncMode() {
  ...
  #onKeyDown(e2) {
    if (e2.ctrlKey && (e2.key === "ArrowUp" || e2.key === "ArrowDown")) {
      if (this.rootElement) {
        e2.preventDefault();
        const fontSize = parseFloat(getComputedStyle(this.contentElement).fontSize);
        const delta = (e2.key === "ArrowUp" ? -this.#settings.lineHeight : this.#settings.lineHeight) * this.#settings.altArrowScrollLines * fontSize;
    ...
    if (e2.altKey && (e2.key === "2" || e2.key === "3")) {
      ...
      e2.preventDefault();
      const html = e2.key === "2" ? `<hr data-manual-anchor='A' class="manual-anchor">` : `<hr data-manual-anchor='B' class="manual-anchor">`;
  ...
  async #onPaste(e2) {
    ...
    e2.preventDefault();
    let isHTML = true;
    let data = e2.clipboardData?.getData("text/html") ?? "";
    if (!data) {
      isHTML = false;
  ...
  getTokenSpanForRange(range) {
    ...
    let r2 = range;
    if (r2.collapsed && r2.endContainer.nodeType === 3 && r2.endOffset === (r2.endContainer.nodeValue?.length ?? 0)) {
      const adj = findAdjacentTextNode(r2.endContainer, true);
      if (adj) {
        const clone = r2.cloneRange();
        clone.setEnd(adj, 0);
        r2 = clone;
      }
    }
    ...
    if (r2.collapsed) {
      let i2 = n2;
      {
        ...
        while (lo <= hi) {
          const mid = lo + hi >> 1;
          const t22 = tokens[mid];
          SetStartEndFromTokenRange(tokenRange, t22);
          const cmp = tokenRange.compareBoundaryPoints(Range.START_TO_END, r2);
          if (cmp > 0) {
            i2 = mid;
            hi = mid - 1;
          } else {
      ...
      const cmpStart = tokenRange.compareBoundaryPoints(Range.END_TO_START, r2);
      return cmpStart <= 0 ? {
        start: i2,
        end: i2 + 1
      } : {
    ...
    if (start === n2) return null;
    let end = n2;
    {
      let lo = start, hi = n2 - 1;
      while (lo <= hi) {
        const mid = lo + hi >> 1;
        const t2 = tokens[mid];
        SetStartEndFromTokenRange(tokenRange, t2);
        const cmp = tokenRange.compareBoundaryPoints(Range.END_TO_START, r2);
        if (cmp >= 0) {
          end = mid;
          hi = mid - 1;
        } else {
  ...
  getOrInsertDiffElement(tokenIndex, hint) {
    ...
    if (tokenIndex === 0) {
      ...
      let a2 = prevToken.endNode;
      let b2 = nextToken.startNode;
      let da = 0, db = 0;
      for (let n2 = a2; n2 && n2 != root; n2 = n2.parentNode) da++;
      for (let n2 = b2; n2 && n2 != root; n2 = n2.parentNode) db++;
...
class DiffContext {
  ...
  // leftImageMap: Map<Token, ImageLoadResult>;
  // rightImageMap: Map<Token, ImageLoadResult>;
  // imageComparisons: Record<string, { similarity: number | undefined }>;
  constructor(leftTokens, rightTokens, diffOptions, entries, leftEntries, rightEntries, diffs, anchorPairs) {
  ...
  resolveMatchingSpanPair(side, sourceSpan) {
    ...
    const n2 = thisEntries.length;
    if (n2 === 0 || sourceSpan.start === 0 && sourceSpan.end === 0) {
      return {
        left: {
          start: 0,
          end: 0
    ...
    if (sourceSpan.start < 0 || sourceSpan.end < sourceSpan.start || sourceSpan.end > n2) {
    ...
    const expandOnSide = (fromSide, span) => {
      ...
      if (a2 >= entries.length) {
        realStart = realEnd = entries.length;
      } else if (a2 === b2) {
        realStart = entries[a2][fromSide].start;
        if (realStart < a2) {
          realEnd = entries[a2][fromSide].end;
        } else {
          realEnd = a2;
        }
      } else {
        realStart = entries[a2][fromSide].start;
        realEnd = entries[b2 - 1][fromSide].end;
      }
      return {
        start: realStart,
        end: realEnd
    ...
    if (expanded.start === expanded.end) {
      const k2 = expanded.start;
      if (k2 >= thisEntries.length) {
        const startAndEnd = thisEntries[thisEntries.length - 1]?.[other]?.end ?? 0;
        otherSpan = {
          start: startAndEnd,
          end: startAndEnd
        };
      } else if (thisEntries[k2] && thisEntries[k2][other]) {
        otherSpan = {
          start: thisEntries[k2][other].start,
          end: thisEntries[k2][other].start
        };
      } else {
        otherSpan = {
          start: 0,
  ...
  async process({
    ...
    if (entries.length > 0) {
      ...
      for (const entry of entries) {
        ...
        for (let i2 = leftSpan.start; i2 < leftSpan.end; i2++) {
          leftEntries[i2] = entry;
        }
        for (let j2 = rightSpan.start; j2 < rightSpan.end; j2++) {
          rightEntries[j2] = entry;
        }
        if (entry.type === 0) {
    ...
    return new DiffContext(leftTokens, rightTokens, diffOptions, entries, leftEntries, rightEntries, diffs, anchorPairs);
  }
...
function mergeRects(rects, toleranceX = 0, toleranceY = 0) {
  rects.sort((a2, b2) => a2.y - b2.y || a2.x - b2.x);
  const merged = [];
  ...
  for (let i2 = 0; i2 < rects.length; i2++) {
    if (used[i2]) continue;
    let base = rects[i2];
    for (let j2 = i2 + 1; j2 < rects.length; j2++) {
      if (used[j2]) continue;
      const compare = rects[j2];
...
class EditorRegion {
  ...
  updateLayout() {
    ...
    if (this.regionX !== x || this.regionY !== y2 || this.regionWidth !== width || this.regionHeight !== height) {
  ...
  prepare(dirtyFlags) {
    ...
    for (let diffIndex = 0; diffIndex < diffs.length; diffIndex++) {
      ...
      if (!geometry) {
        ...
        const x = wholeRect.x + offsetLeft - diffExpandX, y2 = wholeRect.y + offsetTop - diffExpandY, width = wholeRect.width + diffExpandX * 2, height = wholeRect.height + diffExpandY * 2;
  ...
  renderDiffLayer(dirtyFlags) {
    ...
    for (const diffLineRect of this.#diffLineRects) {
      const x = Math.floor(diffLineRect.x - scrollLeft), y2 = Math.floor(diffLineRect.y - scrollTop), width = Math.ceil(diffLineRect.width), height = Math.ceil(diffLineRect.height);
      if (y2 + height < 0) continue;
      if (y2 > regionHeight) break;
    ...
    for (const diffIndex of diffsToRender) {
      ...
      for (const rect of geometry.rects) {
        const x = Math.floor(rect.x - scrollLeft), y2 = Math.floor(rect.y - scrollTop), width = Math.ceil(rect.width), height = Math.ceil(rect.height);
        if (y2 + height < 0) continue;
        if (y2 > regionHeight) break;
        ctx.fillRect(x, y2, width, height);
      }
  ...
  #renderMinimap(ctx) {
    ...
    for (const geometry of this.diffGeometries) {
      const h2 = (geometry.maxY - geometry.minY) * scale;
      roughSpans.push({
        y: geometry.minY * scale,
        h: h2 > 1 ? h2 : 1
      });
    }
    roughSpans.sort((a2, b2) => a2.y - b2.y);
    const TOLERANCE = 1;
    ctx.fillStyle = this.renderer.options.minimapColor;
    let y0 = -Infinity;
    let h0 = 0;
    const x = this.regionWidth - minimapWidth;
    for (let i2 = 0; i2 < roughSpans.length; i2++) {
      const {
        y: y2,
        h: h2
      } = roughSpans[i2];
      if (y2 <= y0 + h0 + TOLERANCE) {
        const bottom = Math.max(y0 + h0, y2 + h2);
        h0 = bottom - y0;
      } else {
        if (h0 > 0) {
  ...
  #mergeIntoDiffLineRects(incoming) {
    ...
    allRects.sort((a2, b2) => a2.y - b2.y);
    this.#diffLineRects.length = 0;
    let current = allRects[0];
    for (let i2 = 1; i2 < allRects.length; i2++) {
      const next = allRects[i2];
  ...
  hitTest(x, y2) {
    ...
    for (const diffIndex of this.#visibleDiffIndicesArr) {
      ...
      if (geometry && geometry.rects) {
        for (const rect of geometry.rects) {
          if (x >= rect.x && x <= rect.x + rect.width && y2 >= rect.y && y2 <= rect.y + rect.height) {
            return diffIndex;
          }
        }
      }
...
class Renderer {
  ...
  #updateHighlightedDiffIndex() {
    ...
    const l2 = this.#leftRegion.setHighlightedDiffIndex(actualDiffIndex);
    const r2 = this.#rightRegion.setHighlightedDiffIndex(actualDiffIndex);
    this.#nextRenderFlags |= l2 | r2 << REGION_FLAGS_SHIFT;
  }
  get isSyncMode() {
  ...
  updateMousePosition(x, y2) {
    this.#mouseX = x - this.#canvasX;
    this.#mouseY = y2 - this.#canvasY;
  ...
  hitTest(x, y2) {
    let diffIndex = null;
    if (x < 0 || y2 < 0) ;
    else if (x > this.#canvasWidth || y2 > this.#canvasHeight) ;
    else {
      let region = null;
      if (x >= this.#leftRegion.regionX && x < this.#leftRegion.regionX + this.#leftRegion.regionWidth && y2 >= this.#leftRegion.regionY && y2 < this.#leftRegion.regionY + this.#leftRegion.regionHeight) {
      ...
      let guideLineY = -1;
      if (region) {
        diffIndex2 = region.hitTest(x - region.regionX, y2 - region.regionY);
        guideLineY = y2 - region.regionY;
      }
...
class DiffseekRuntime extends EventTarget {
  workspaceEl;
  deckEl;
  leftEditor;
  rightEditor;
  ...
  #diffState = null;
  constructor(workspaceEl2, options) {
    super();
    this.workspaceEl = workspaceEl2;
    workspaceEl2.classList.add("ds-workspace");
    this.setOptions(options);
    workspaceEl2.style.setProperty("--editor-minimap-width", `${15}px`);
    this.deckEl = document.createElement("div");
    this.deckEl.className = "ds-deck";
    workspaceEl2.appendChild(this.deckEl);
  ...
  #setupCallbacks() {
    const callbacks = {
      ...
      mouseMove: (editor, e2) => this.#handleEditorMouseMove(editor, e2),
      mouseLeave: (editor, e2) => this.#handleEditorMouseLeave(editor, e2),
      focus: (editor) => {
  ...
  #dispatchDiffStateChanged() {
    this.dispatchEvent(new CustomEvent("diffStateChanged", {
      detail: this.#diffState ? {
        ...this.#diffState
      } : null
    }));
  }
  #handleEditorContentChanging(editor) {
    ...
    if (this.#diffState?.status !== "tokenizing") {
      this.#diffState = {
        status: "tokenizing",
        progress: 0
      };
      this.#dispatchDiffStateChanged();
    }
  }
  ...
  #handleEditorMouseMove(_editor, e2) {
    this.renderer.updateMousePosition(e2.clientX, e2.clientY);
  }
  #handleEditorMouseLeave(_editor, _e) {
  }
  #handleDiffWorkerStatus = (e2) => {
    if (e2.type === "progress") {
      this.#diffState = {
        status: "diffing",
  ...
  #handleRendererDiffVisibilityChanged = (changes) => {
    ...
    this.dispatchEvent(new CustomEvent("diffVisibilityChanged", {
      detail: {
        changes
      }
    }));
  ...
  set syncMode(value) {
    ...
    const workspaceEl2 = document.getElementById("diffseek-workspace");
    this.#syncMode = value;
    this.leftEditor.isSyncMode = value;
    this.rightEditor.isSyncMode = value;
    this.renderer.isSyncMode = value;
    workspaceEl2.classList.toggle("sync-mode", value);
    if (value) {
  ...
  async #startDiffWorkflow() {
    ...
    try {
      ...
      this.#dispatchDiffStateChanged();
      const leftTokens = this.leftEditor.tokens.map((t2) => ({
        text: t2.text,
        flags: t2.flags
      }));
      const rightTokens = this.rightEditor.tokens.map((t2) => ({
        text: t2.text,
        flags: t2.flags
      }));
  ...
  async #alignAnchors() {
    requestAnimationFrame(() => {
      ...
      for (let i2 = 0; i2 < anchorPairs.length; i2++) {
        const pair = anchorPairs[i2];
        const {
          leftEl,
          rightEl
        } = pair;
  ...
  #setDiffContext(diffContext) {
    if (this.#diffContext !== diffContext) {
      ...
      this.dispatchEvent(new CustomEvent("diffContextChanged", {
        detail: {
          diffContext: this.#diffContext ?? null
        }
      }));
    }
  }
...
const appState = {
  diffContext: d(null),
  diffOptions: d(getDefaultDiffOptions()),
  optionsModalOpen: d(false),
  visibleDiffs: d({
    left: /* @__PURE__ */ new Set(),
    right: /* @__PURE__ */ new Set()
  }),
...
function AppHeader() {
  ...
  const state = appState.diffState.value;
  const diffContext = appState.diffContext.value;
  const diffOptions = appState.diffOptions.value;
  let t0;
  if ($[0] === /* @__PURE__ */ Symbol.for("react.memo_cache_sentinel")) {
    t0 = (value) => {
      ...
      appState.diffOptions.value = nextOptions;
    };
    $[0] = t0;
  } else {
    t0 = $[0];
  }
  const setWhitespace = t0;
  let t1;
  if ($[1] === /* @__PURE__ */ Symbol.for("react.memo_cache_sentinel")) {
    t1 = /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "app-header-left", children: diffContext?.isValid ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
        "왼쪽: ",
        diffContext.leftTokens.length
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
        "오른쪽: ",
        diffContext.rightTokens.length
      ] })
    ] }) : /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "DiffSeek" }) });
    $[1] = t1;
  } else {
    t1 = $[1];
  }
  ...
  if ($[2] === /* @__PURE__ */ Symbol.for("react.memo_cache_sentinel")) {
    t2 = /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "공백" });
    $[2] = t2;
  } else {
    t2 = $[2];
  }
  ...
  if ($[3] === /* @__PURE__ */ Symbol.for("react.memo_cache_sentinel")) {
    t3 = /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "app-header-select", "aria-label": "공백 처리", children: [
      t2,
      /* @__PURE__ */ jsxRuntimeExports.jsxs("select", { value: diffOptions.whitespace, onChange: (e2) => setWhitespace(e2.target.value), children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "collapse", children: "기본" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "ignore", children: "무시" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "ignoreAtEdge", children: "가장자리만" })
      ] })
    ] });
    $[3] = t3;
  } else {
  ...
  if ($[4] === /* @__PURE__ */ Symbol.for("react.memo_cache_sentinel")) {
    t4 = /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "app-header", children: [
      t1,
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "app-header-right", children: [
        t3,
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "app-header-status", children: state ? /* @__PURE__ */ jsxRuntimeExports.jsx(StateText, { state }) : diffContext && diffContext.isValid ? /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
...
function StateText(t0) {
  ...
  switch (state.status) {
    case "diffing": {
      const t1 = Math.floor(state.progress);
      let t2;
      if ($[0] !== t1) {
        t2 = /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
          "비교 중... ",
          t1,
          "%"
        ] });
    ...
    case "tokenizing": {
      let t1;
      if ($[2] === /* @__PURE__ */ Symbol.for("react.memo_cache_sentinel")) {
        t1 = /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "비교 중..." });
        $[2] = t1;
      } else {
        t1 = $[2];
      }
    ...
    case "processing": {
      let t1;
      if ($[3] === /* @__PURE__ */ Symbol.for("react.memo_cache_sentinel")) {
        t1 = /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "마무리 중..." });
        $[3] = t1;
      } else {
        t1 = $[3];
      }
...
const dsRuntime = new DiffseekRuntime(workspaceEl, {});
dsRuntime.setDiffOptions({
  // ...
});
function App() {
  ...
  if ($[2] === /* @__PURE__ */ Symbol.for("react.memo_cache_sentinel")) {
    t2 = /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "app-header-container", children: /* @__PURE__ */ jsxRuntimeExports.jsx(AppHeader, {}) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("main", { id: "diffseek-host", ref: hostRef })
    ] });
    $[2] = t2;
  } else {
    t2 = $[2];
...
clientExports.createRoot(document.getElementById("root")).render(/* @__PURE__ */ jsxRuntimeExports.jsx(reactExports.StrictMode, { children: /* @__PURE__ */ jsxRuntimeExports.jsx(MantineProvider, { children: /* @__PURE__ */ jsxRuntimeExports.jsx(App, {}) }) }));
