...
describe("plain text vs table: tokenization", () => {
	it("last text token in each TD has LINE_END", async () => {
		...
		const token_대출개요 = textTokens.find(
			(t) => result.wholeText.slice(t.textOffset, t.textOffset + t.textLength) === "대출개요",
		);
		...
		const token_개인택시 = textTokens.find(
			(t) => result.wholeText.slice(t.textOffset, t.textOffset + t.textLength) === "개인택시",
		);
...
describe("plain text vs table: full pipeline", () => {
	...
	it("DiffEntry invariants hold", async () => {
		...
		for (const diff of diffContext.diffs) {
			...
			// span validity
			expect(diff.leftSpan.end, `diff[${diff.diffIndex}] leftSpan valid`).toBeGreaterThanOrEqual(
				diff.leftSpan.start,
			);
			expect(diff.rightSpan.end, `diff[${diff.diffIndex}] rightSpan valid`).toBeGreaterThanOrEqual(
				diff.rightSpan.start,
			);
		}
	});
