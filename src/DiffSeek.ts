...
class DiffSeek {
	...
	#setupEventListeners() {
		...
		document.addEventListener("click", (e) => {
			if (e.target instanceof HTMLElement && e.target.closest(".diff-item")) {
				if (e.ctrlKey) {
					this.#copySelectionToClipboard(e, true);
				}
			}
		});
		...
		document.addEventListener(
			...
			(e) => {
				...
				if (e.shiftKey && e.ctrlKey && (e.key === "c" || e.key === "C")) {
					...
					this.#copySelectionToClipboard(e);
				}
			},
			true
		);
	...
	#copySelectionToClipboard(e: Event, draging: boolean = false): boolean {
		...
		if (draging && e.target instanceof HTMLElement && e.target.matches(".diff-item")) {
		...
		if (leftSpan && rightSpan) {
			console.log("Copying selection to clipboard:", leftSpan, rightSpan);
			function replaceTabsWithSingleSpace(str: string) {
				return str.replace(/\t+/g, ' ');
			}

			const leftText = replaceTabsWithSingleSpace(this.#leftEditor.getTokenRange(leftSpan.index, leftSpan.count).toString().trim());
			const rightText = replaceTabsWithSingleSpace(this.#rightEditor.getTokenRange(rightSpan.index, rightSpan.count).toString().trim());
			const leftSectionLabel = replaceTabsWithSingleSpace(getSectionTrailText(this.#diffContext.leftSectionHeadings, leftSpan.index));
			const rightSectionLabel = replaceTabsWithSingleSpace(getSectionTrailText(this.#diffContext.rightSectionHeadings, rightSpan.index));
			const payload = `${leftSectionLabel}\t${rightSectionLabel}\t${leftText}\t${rightText}`;
