...
export class DiffseekEngine {
	...
	readonly diffVisibilityChanged = createEvent<{
		left: DiffVisibilityChangeEntry[];
		right: DiffVisibilityChangeEntry[];
	}>();
			...
			if (
				...
				if (sourceSpan && sourceSpan.start !== sourceSpan.end) {
					...
					if (left && right) {
						...
						if (
							this.leftSelectionSpan?.start !== left.start ||
	...
	scrollNudge(editorName: EditorName | "current", direction: "up" | "down", lines?: number) {
		const editor =
			editorName === "current" ? this.focusedEditor : editorName === "left" ? this.leftEditor : this.rightEditor;
		if (editor) {
	...
	getTextForTokenSpan(side: EditorName, span: Span, options?: GetTextForTokenSpanOptions): string | null {
		...
		for (let i = span.start; i < span.end; i++) {
			...
			const end = i + 1 < tokens.length ? tokens[i + 1].textOffset : token.textOffset + token.textLength;
	...
	private async startDiffWorkflow() {
		...
		try {
			...
			this.emitStatusForRun(controller, {
				phase: "tokenizing",
				startedAtMs: t0,
				leftTokenCount,
				rightTokenCount,
...
function isPaletteEqual(a: Readonly<Palette>, b: Readonly<Palette>): boolean {
	return (
		isNumberArrayEqual(a.diffHues, b.diffHues) &&
