...
describe("section heading tokenization", () => {
	...
	it("여러 줄 각각 헤딩 — tokenIndex가 올바름", async () => {
		const { tokens, sectionHeadings } = await tok("<div>1. 제목A</div><div>2. 제목B</div>");
	...
	it("여러 타입 혼합 — tokenIndex가 각각 올바름", async () => {
		const { tokens, sectionHeadings } = await tok("<div>제1조 조문</div><div>(1) 항목</div><div>가. 내용</div>");
		...
		expect(headingFlagsToType(tokens[sectionHeadings[1].tokenIndex].flags)).toBe(
			SECTION_HEADING_TYPE_PAREN_NUMERIC,
		);
	...
	it("structural 토큰 전후 content 토큰은 LINE_END/LINE_START이고 lineNumber가 다르다", async () => {
		...
		for (let i = 0; i < tokens.length; i++) {
			...
			if (pi >= 0 && ni < tokens.length) {
				expect(
					tokens[pi].flags & TOKEN_FLAGS_LINE_END,
					`token[${pi}] should have LINE_END before structural at ${i}`,
				).toBeTruthy();
				expect(
					tokens[ni].flags & TOKEN_FLAGS_LINE_START,
					`token[${ni}] should have LINE_START after structural at ${i}`,
				).toBeTruthy();
				expect(tokens[pi].lineNumber, `lineNumber should differ across structural boundary at ${i}`).not.toBe(
					tokens[ni].lineNumber,
				);
			}
		}
	});
