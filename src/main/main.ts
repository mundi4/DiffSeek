	...
	function recalculateAlignmentPaddingAndPositions() {
		...
		for (let anchorIndex = 0; anchorIndex < anchors.length; anchorIndex++) {
			...
			if (anchor.type === "before") {
				...
				const leftH = leftHeights[anchorIndex],
					rightH = rightHeights[anchorIndex];
				const leftB = leftY + leftH,
					rightB = rightY + rightH;
				delta = leftB - rightB;
			} else {
...
animation: highlightAnimation 0.3s linear 3; 
	...
	function findTokenMapping(leftIndex?: number, rightIndex?: number) {
		if (leftIndex !== undefined || rightIndex !== undefined) {
			...
			let mid = 0;
			let entry: DiffEntry | null = null;
			let side: DiffEntrySide;
			while (lo <= hi) {
				mid = (lo + hi) >>> 1;
				entry = _mappings![mid];
