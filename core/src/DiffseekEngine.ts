...
import type { DiffContext, DiffOptions, AnchorPair, EditorName, Span, DiffVisibilityChangeEntry, DiffseekEventMap, DiffseekInterface, SerializedToken, Token } from "./types";
...
export class DiffseekEngine implements DiffseekInterface {
    ...
    private async startDiffWorkflow() {
        ...
        try {
            ...
            const STRIDE = 5;
            const serializeTokenArray = (tokens: readonly Token[]) => {
            ...
            const leftTokens = serializeTokenArray(this.leftEditor.tokens);
            const rightTokens = serializeTokenArray(this.rightEditor.tokens);
            ...
            const workerResult = await this.diffWorker.run({
                leftWholeText,
                rightWholeText,
                leftTokenBuffer: leftTokens,
                rightTokenBuffer: rightTokens,
                options: diffOptions,
            ...
            const diffContext = await this.diffProcessor.process({
                ...
                leftTokenBuffer: workerResult.leftTokenBuffer,
                rightTokenBuffer: workerResult.rightTokenBuffer,
                diffOptions,
                scheduler,
            ...
            console.log("Diff workflow completed:", diffContext);
        } catch (err) {
