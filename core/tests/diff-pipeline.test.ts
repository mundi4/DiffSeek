...
describe("diff pipeline end-to-end", () => {
	it("identical documents produce zero diffs", async () => {
		const { diffContext } = await runFullPipeline("<p>hello world</p>", "<p>hello world</p>");
		expect(diffContext.diffs.length).toBe(0);
	});

	it("simple text change produces diffs with correct side assignment", async () => {
		const { diffContext, leftEl, rightEl } = await runFullPipeline("<p>hello world</p>", "<p>hello earth</p>");
	...
	it("added content on right: leftSpan is empty, rightSpan is filled", async () => {
		const { diffContext } = await runFullPipeline("<p>hello</p>", "<p>hello world</p>");

		expect(diffContext.diffs.length).toBeGreaterThan(0);

		// At least one diff should have an empty left side (pure addition)
		const addedDiffs = diffContext.diffs.filter(
			(d) => d.leftSpan.start === d.leftSpan.end && d.rightSpan.end > d.rightSpan.start,
		);
		expect(addedDiffs.length).toBeGreaterThan(0);
	});

	it("removed content on left: rightSpan is empty, leftSpan is filled", async () => {
		const { diffContext } = await runFullPipeline("<p>hello world</p>", "<p>hello</p>");

		expect(diffContext.diffs.length).toBeGreaterThan(0);

		// At least one diff should have an empty right side (pure removal)
		const removedDiffs = diffContext.diffs.filter(
			(d) => d.rightSpan.start === d.rightSpan.end && d.leftSpan.end > d.leftSpan.start,
		);
...
describe("diff pipeline: text content edge cases", () => {
	it("whitespace-only differences", async () => {
		const { diffContext, leftEl, rightEl } = await runFullPipeline("<p>hello  world</p>", "<p>hello world</p>");
	...
	it("content added at document start", async () => {
		const { diffContext, leftEl, rightEl } = await runFullPipeline("<p>world</p>", "<p>hello world</p>");
	...
	it("content added at document end", async () => {
		const { diffContext, leftEl, rightEl } = await runFullPipeline("<p>hello</p>", "<p>hello world</p>");
...
describe("diff pipeline: empty document edge cases", () => {
	it("left empty, right has content", async () => {
		const { diffContext, leftEl, rightEl } = await runFullPipeline("", "<p>content</p>");
	...
	it("single character difference", async () => {
		const { diffContext, leftEl, rightEl } = await runFullPipeline("<p>a</p>", "<p>b</p>");
