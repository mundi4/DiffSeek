...
- 공백 무시. 공백을 완전히 무시하는건 완전히 의도된 것임. **업무매뉴얼의 줄바꿈 위치는 절대로 문장의 끝 혹은 단락의 끝과 일치하지 않음!** 공백을 전혀 무시하지 않거나 정규화(여러개의 연속된 공백을 하나로 인식)해서 비교하는 코드를 짜는게 100만배 쉽고 계산속도도 훨씬 빠름. (토글 가능: `F8`)
...
- HTML 없이 텍스트만으로 보여주는 걸로 시작했지만 결국 HTML 단계까지 와버렸음.
...
- 그랬지만... HTML 복붙을 지원하게 된 순간부터 붙여넣기는 반드시 개입해야 함. `document.execCommand("insertHTML",...)`를 쓰면 undo/redo 문제 없이 작동하지만 이 함수는 정말 끔찍하리만큼 느림. 레거시 함수라서 버려둔 듯. 어쩔 수 없이 큰 문서를 붙여넣을 땐 undo로 그 붙여넣기 이전 상태로 돌아가는 건 포기.
...
- html + css + js는 어쩔 수 없는 선택. 쌩html 쌩css 쌩js로 구현
- 섬세하고 이쁜 UI 요소가 부족한 것은 절대적으로 이 때문임.
...
- 워크비 쪽지/내부메일로 보내고 회사에서 열고 메모장에 붙여넣기. 고로 코드가 너무 커지면 안됨.
...
### 단축키?
- F2 - 정렬 모드 토글(양쪽 줄맞춤, 스크롤 동기화, 편집 불가)
- F8 - 비교 시 공백 무시/줄바꿈 경계에서만 공백 무시/공백 정규화 토글
