...
async function tok(html: string) {
	...
	const texts = result.tokens.map((t) => result.wholeText.slice(t.textOffset, t.textOffset + t.textLength));
	return { ...result, texts };
}
...
describe("structural tokens", () => {
	it("TD open/close 구분", async () => {
		...
		const tdOpen = structural.filter(
			(t) => isStructuralOpen(t.flags) && getStructuralElementType(t.flags) === STRUCTURAL_ELEMENT_TD,
		);
		const tdClose = structural.filter(
			(t) => isStructuralClose(t.flags) && getStructuralElementType(t.flags) === STRUCTURAL_ELEMENT_TD,
		);
	...
	it("TR open/close 구분", async () => {
		const r = await tok("<table><tr><td>내용</td></tr></table>");
		const structural = r.tokens.filter((t) => t.flags & TOKEN_FLAGS_TYPE_STRUCTURAL);
		const trOpen = structural.filter(
			(t) => isStructuralOpen(t.flags) && getStructuralElementType(t.flags) === STRUCTURAL_ELEMENT_TR,
		);
		const trClose = structural.filter(
			(t) => isStructuralClose(t.flags) && getStructuralElementType(t.flags) === STRUCTURAL_ELEMENT_TR,
		);
...
describe("complex structures", () => {
	...
	it("colspan TD 다음 행의 다중 TD", async () => {
		const r = await tok(
			'<table><tr><td colspan="2">hello world</td></tr><tr><td>1.대출개요</td><td>개인택시</td></tr></table>',
		);
		const tt = textTokens(r);
