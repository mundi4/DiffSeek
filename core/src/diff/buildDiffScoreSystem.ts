...
export function buildDiffScoreSystem(
    opt: HistogramAnchorScoreOptions = {}
): DiffScoreSystem {
    ...
    const freqBase =
        (opt.freqBase ?? [64, 28, 12, 4]).slice(0, freqGradeCount);

    const lenBase =
        (opt.lenBase ?? [1, 2, 3, 5, 8, 12, 17, 24]).slice(0, lenGradeCount);

    const policyWeights =
        opt.policyWeights ?? [0, 0.05, 0.12];

    const positionalWeights =
        opt.positionalWeights ?? [0, 0.03, 0.07];

    // ---------- freq LUT ----------
    const freqPairGradeLUT =
        new Uint8Array(freqStride * freqStride);
    ...
    for (let a = 0; a <= freqMax; a++) {
        for (let b = 0; b <= freqMax; b++) {
            ...
            if (grade >= freqGradeCount)
                grade = freqGradeCount - 1;
    ...
    // ---------- core raw ----------
    const raw =
        new Uint32Array(freqGradeCount * lenGradeCount);
    ...
    // ---------- core normalize ----------
    const coreScoreTable =
        new Uint16Array(raw.length);

    const freqRowBase =
        new Uint16Array(freqGradeCount);
    ...
    for (let f = 0; f < freqGradeCount; f++) {
        ...
        for (let l = 0; l < lenGradeCount; l++) {
            const idx = base + l;
            coreScoreTable[idx] =
                (raw[idx] * coreMaxScore / rawMax) | 0;
        }
    }

    // ---------- policy (coreMax 기준 비율) ----------
    const policyTable =
        new Uint16Array(policyWeights.length);
